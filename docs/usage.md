# nf-core/crispr: Usage

## :warning: Please read this documentation on the nf-core website: [https://nf-co.re/crispr/usage](https://nf-co.re/crispr/usage)
...
```bash
nextflow run nf-core/crispr --input ./samplesheet.csv --outdir ./results --genome GRCh37 -profile docker
```
...
```bash
nextflow run nf-core/crispr -profile docker -params-file params.yaml
```
...
First, go to the [nf-core/crispr releases page](https://github.com/nf-core/crispr/releases) and find the latest pipeline version - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`. Of course, you can switch to another version by changing the number after the `-r` flag.
...
Every run also writes `pipeline_info/pipeline_manifest.config` when it reaches
workflow completion, whether the final state is successful or failed. The file
is valid Nextflow config syntax. Its `manifest` block records the repository
URL, branch, exact Git commit, and short manifest version; comments preserve
the full launch command (`pipeline_command`), `nextflow_version`, run name,
completion time, and run status. It is generated by the pipeline itself and
does not replace the config files
