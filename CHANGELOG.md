...
- **Both methods now condition on the same covariate list**, named once in `bin/inference_covariates.py`: the mitochondrial fraction (accepted as `percent_mito`, `pct_counts_mt`, `percent.mito`, `pct_mito` or `mito_frac`) and the batch column.
- **PerTurbo swaps a covariate.** v1 passed `continuous_covariates_keys=["log1p_total_guide_umis_centered"]` plus `batch_key="batch"` (`refs/remotes/origin/main:bin/perturbo_inference.py:158-163`); the guide-UMI term is gone and `percent_mito` takes its place.
- **SCEPTRE gains one.** Its R reader has always fed the MuData's top-level `obs` to `import_data` as `extra_covariates`, and that frame is the intersection of the gene and guide modalities' `obs` columns (`bin/create_mdata.py:377-383`, unchanged from v1), which contains `batch` but never `percent_mito`, since the mitochondrial fraction lives only in `gene.obs`. `bin/prepare_inference.py:144` now writes the agreed list into that frame explicitly.
- The library size is deliberately absent: PerTurbo takes it as an offset and SCEPTRE adds `log(response_n_umis)` itself.
- A comment in `bin/inference_covariates.py:27-29` states that SCEPTRE declines any factor with fifteen or more levels (`MAX_N_LEVELS_ALLOWED` in its formula builder), so on a screen with dozens of sequencing batches it would still decline the batch term PerTurbo uses. That is a claim about the SCEPTRE package in `sjiang9/sceptre-igvf:0.2`, and nothing in this repository verifies it.
