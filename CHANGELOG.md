...
- Pin PerTurbo to `v2.0.0rc11` (`sha256:e51d5e9bc70971610fc9d259d48f83bff96e10723cf21a0f52a4345a07ba57c5`). rc11 is rc10 plus three changes. (1) The Chernoff tail guard on the equal-tail propensity saddlepoint: each interior root's residual and the raw Lugannani-Rice correction are checked, failed approximations fall back to a guarded Chernoff bound or a conservative p=1, and the residual tolerance is 1e-3 null sd, since at 1e-6 the guard replaced saddlepoint values accurate to 1e-4 nats with bounds about 3 nats looser on a knife edge that two propensity solvers agreeing to 8e-7 in probability fell on opposite sides of. (2) A first log line naming the code that is running (`[perturbo] perturbo <version> running from <path>`), because `importlib.metadata` reports the installed version even when a source tree shadows it. (3) Stage-two gene blocks sized to the device: a 1,060,779-cell screen with 47 guides per cell asked XLA for a 47.6 GiB allocation and died after the CRT had finished; the block now halves from 256 until the gather fits a third of the card. Validation: on the TAP-seq chr8 panel with the shared covariates the guard takes global guide-level PerTurbo calls from 1,546 (rc10) to 69 while keeping all 21 recovered paper pairs and raising cis agreement with SCEPTRE to Jaccard 0.91 (50 vs 51 calls, 48 shared); element-level calls 75 -> 45. On Replogle genome-wide the guard at the old 1e-6 tolerance was log10 p Pearson 0.9994 against the 8 Sep run with q<0.1 calls 623,925 -> 612,399, and 7,809 of the 11,637 lost calls were the residual-guard artifact the 1e-3 tolerance removes (623,981 with it, Jaccard 0.997). The four tail-guard columns (`perturbo_crt_tail_failure_reason`, `perturbo_crt_used_chernoff`, `perturbo_crt_used_conservative_one`, `perturbo_crt_root_residual_null_sd`) reach the result tables from this pin onward.
