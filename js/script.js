...
function filterTable(container, searchTerm, columnsToSearch = [1, 2, 3]) {
    ...
    rows.forEach((row, index) => {
        const cells = columnsToSearch.map(colIndex => {
            const cell = row.querySelector(`td:nth-child(${colIndex})`);
            const cellText = cell ? cell.textContent.trim() : '';
            console.log(`Row ${index + 1}, Column ${colIndex} content:`, cellText); // Debugging
            return cellText;
        });

        if (cells.some(cell => searchRegex.test(cell))) {
            row.style.display = '';
...
// Function to store the original order of rows
function storeOriginalOrder(tableContainer) {
    const tbody = tableContainer.querySelector('tbody');
    originalRows = Array.from(tbody.querySelectorAll('tr'));
}

// handle sorting column function
// Modified sorting function
function sortTableByColumn(tableContainer, columnName, ascending = true) {
    if (!originalRows) {
        storeOriginalOrder(tableContainer);
    }
    ...
    columnIndex = headers.findIndex(th => th.textContent.trim().includes(columnName)) + 1;

    if (columnIndex === 0) {
        console.error(`Cannot find a column with the name: ${columnName}`);
    ...
    rows.sort((a, b) => {
        ...
        if (!aCell || !bCell) {
            console.error('Cannot find the table cell at the given index.');
            return 0;
        }
        ...
        const aValue = parseFloat(aText) || aText;
        const bValue = parseFloat(bText) || bText;

        // Handle 'inf' and '-inf' values explicitly
        if (aValue === Infinity || aText === 'inf') return ascending ? 1 : -1;
        if (bValue === Infinity || bText === 'inf') return ascending ? -1 : 1;
        if (aValue === -Infinity || aText === '-inf') return ascending ? -1 : 1;
        if (bValue === -Infinity || bText === '-inf') return ascending ? 1 : -1;
    ...
    // Reapply pagination if needed
    const visibleRowCount = rows.filter(row => row.style.display !== 'none').length;
    if (typeof updatePagination === 'function') {
        updatePagination(tableContainer, visibleRowCount);
        showPage(tableContainer, 1);
    }
}

// reset sorting
// New function to reset the table to its original order
function resetTableSort(tableContainer) {
    if (!originalRows) {
        console.error('Original row order not stored. Cannot reset.');
        return;
    }
    ...
    // Re-append the original rows
    originalRows.forEach(row => tbody.appendChild(row.cloneNode(true)));

    // Reapply pagination if needed
    const visibleRowCount = originalRows.filter(row => row.style.display !== 'none').length;
    if (typeof updatePagination === 'function') {
        updatePagination(tableContainer, visibleRowCount);
        showPage(tableContainer, 1);
    }
...
function initializeSceptreInferenceTable(tableContainer) {
    storeOriginalOrder(tableContainer);
    ...
    sortIcons.forEach(icon => {
        icon.addEventListener('click', function() {
            const columnName = this.closest('th').textContent.trim();
            const ascending = this.textContent === '▲';
            sortTableByColumn(tableContainer, columnName, ascending);
        });
    });
}
...
document.addEventListener('DOMContentLoaded', function() {
    const sceptreInferenceTables = document.querySelectorAll('.sceptre-inference-table');
    sceptreInferenceTables.forEach(initializeSceptreInferenceTable);
});
...
function initializeAllTableSearch() {
    document.querySelectorAll('.table-search').forEach(searchInput => {
        searchInput.addEventListener('input', function() {
            ...
            let columnsToSearch = [1]; // Default to first column

            if (placeholder.includes('gene_id') || placeholder.includes('guide_id') || placeholder.includes('intended_target_name')) {
                columnsToSearch = [1, 2, 3]; // Search in gene_id, guide_id, intended_target_name
            }

            console.log('Searching in columns:', columnsToSearch); // Log for debugging
            filterTable(container, this.value, columnsToSearch);
        });
    });
}
