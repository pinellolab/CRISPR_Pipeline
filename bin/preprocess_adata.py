...
def plot_barcode_rank(knee_df, point_1, point_2, outpath):
    ...
    if point_1 is not None:
        ...
        plt.axvline(
            knee_rank, color="red", linestyle="--", alpha=0.8, label="Knee 1"
        )
...
def main(
    ...
    gname_rna_path = os.path.join(
        gname_rna, f"{folder_}/cells_x_genes.genes.names.txt"
    )
    ...
    if len(gene_names) == adata_rna.shape[1]:
        ...
        raise ValueError(
            "The number of gene names does not match the number of variables in adata_rna"
        )
    ...
    knee_df = pd.DataFrame(
        {"sum": cell_counts, "barcodes": adata_rna.obs_names.values}
    )
    ...
    if barcode_filter != "none":
        ...
        if selected_point is None:
            print(
                "Barcode filtering skipped: knee point could not be determined."
            )
        else:
            ...
            print(
                f"Barcode filtering using rank {knee_rank} (count >= {count_threshold:.0f})"
            )
    ...
    adata_rna.var["ribo"] = adata_rna.var["symbol"].str.startswith(
        ("RPS", "RPL")
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "gname_rna", type=str, help="Path to the cells x genes txt file."
    )
    parser.add_argument(
        "--min_genes",
        type=int,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.2,
        help="Minimum percent of proportion of mitochondrial reads in cells.",
    )
    parser.add_argument(
        "--reference", type=str, required=True, help="Reference species"
    )
    parser.add_argument(
        "--barcode-filter",
