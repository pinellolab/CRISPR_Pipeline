...
def series_to_anndata(count_series: pd.Series, args) -> anndata.AnnData:
    ...
    # to the wrong guide identifier.
    guide_set_df = pd.read_csv(str(args.guide_set_fn), sep='\t')
    if guide_set_df['spacer'].duplicated().any():
        duplicated = guide_set_df.loc[guide_set_df['spacer'].duplicated(False), 'spacer'].unique()
        raise ValueError(f"Guide metadata contains duplicated spacer sequences: {duplicated[:10].tolist()}")
    spacer_to_guide = guide_set_df.set_index('spacer')['guide_id']
    missing = adata.var_names.difference(spacer_to_guide.index)
    if len(missing):
        raise ValueError(f"Mapped protospacers absent from guide metadata: {missing[:10].tolist()}")
    adata.var['guide_id'] = spacer_to_guide.loc[adata.var_names].to_numpy()
    return adata
