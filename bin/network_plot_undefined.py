...
def plot_network(mdata: MuData, central_node: str, method: Optional[Literal['sceptre', 'perturbo']],
               ...
               ax: Optional[plt.Axes] = None, analysis_type: Optional[str] = None):
   ...
   ax.set_title(f"{title_prefix}{method_title} Network - {central_node}")
   return True
...
def select_central_nodes(mdata: MuData, num_nodes: int, source_column: str,
                       ...
                       min_weight: Optional[float] = None, results_key: str = "test_results"):
    ...
    if valid_nodes:
        print(f"Top {len(valid_nodes)} nodes for {method_name} ({results_key}):", valid_nodes)
    else:
        print(f"No valid nodes found for {method_name} ({results_key}) with the current filters")
...
def process_results_config(mdata: MuData, args, results_key: str, analysis_type: Optional[str] = None):
    ...
    if 'log2_fc' in cols and 'p_value' in cols:
        ...
        central_nodes = select_central_nodes(
            mdata, args.num_nodes, args.source_column, args.target_column, None, results_key=results_key
        )
        ...
        fig_title = f"{analysis_type.capitalize()} Networks" if analysis_type else "Networks"
        ...
        output_file = os.path.join(output_dir, f"{filename_prefix}network_plot.png")
        ...
        for method in available_methods:
            print(f"\nGenerating plot for {method} ({results_key})...")

            method_nodes = select_central_nodes(mdata, args.num_nodes, args.source_column, args.target_column, method, args.min_weight, results_key)
