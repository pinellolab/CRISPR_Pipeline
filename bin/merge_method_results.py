...
def merge_method_results(sceptre_per_guide, sceptre_per_element, perturbo_per_guide, perturbo_per_element, base_mudata_path):
    ...
    merged_guide_df.to_csv("merged_per_guide_output.tsv.gz", sep='\t', index=False, compression='gzip')
    merged_element_df.to_csv("merged_per_element_output.tsv.gz", sep='\t', index=False, compression='gzip')
