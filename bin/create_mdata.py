...
def main(
    adata_rna, adata_guide, guide_metadata, gtf, moi, capture_method, adata_hashing=None
):
    # Load the data
    ...
    if len(guide_metadata) != len(adata_guide.var):
        print(
            f"The numbers of sgRNA_ID/guide_id are different: There are {len(adata_guide.var)} in guide anndata, but there are {len(guide_metadata)} in guide metadata."
        )

    if (
        "guide_id" in guide_metadata.columns
        and guide_metadata["guide_id"].duplicated().any()
    ):
        warnings.warn(
            "Duplicate guide_id values detected in guide metadata; merge may drop or duplicate rows.",
            UserWarning,
        )
    if (
        "guide_id" in adata_guide.var.columns
        and adata_guide.var["guide_id"].duplicated().any()
    ):
        warnings.warn(
            "Duplicate guide_id values detected in guide AnnData; merge may drop or duplicate rows.",
            UserWarning,
        )

    adata_guide.var = adata_guide.var.merge(
        guide_metadata, left_on="guide_id", right_on="guide_id", how="inner"
    )
    ...
    obs_names = set(mdata.mod["guide"].obs.columns.tolist()) & set(
        mdata.mod["gene"].obs.columns.tolist()
    )
    if adata_hashing is not None:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Create a MuData object from scRNA and guide data."
    )
    ...
    parser.add_argument(
        "guide_metadata", type=str, help="Path to the guide metadata tsv file."
    )
    parser.add_argument("gtf", type=str, help="Path to the GTF file.")
    parser.add_argument(
        "moi", default="", help="Multiplicity of infection (MOI) of the screen."
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to the hashing AnnData file (optional).",
    )
