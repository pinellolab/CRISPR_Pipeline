...
process inference_perturbo_trans {
    ...
    script:
        ...
        perturbo_inference_chunked.py ${mudata} perturbo_trans_per_element_output.tsv.gz --chunk_size ${params.INFERENCE_PERTURBO_TRANS_MAX_GENES_PER_CHUNK} --batch_size ${params.INFERENCE_PERTURBO_BATCH_SIZE} --num_workers ${PERTURBO_NUM_WORKERS} --efficiency_mode ${efficiency_mode} --inference_type element --test_all_pairs
        
        # Run PerTurbo inference for per-guide results  
        perturbo_inference_chunked.py ${mudata} perturbo_trans_per_guide_output.tsv.gz --chunk_size ${params.INFERENCE_PERTURBO_TRANS_MAX_GENES_PER_CHUNK} --batch_size ${params.INFERENCE_PERTURBO_BATCH_SIZE} --num_workers ${PERTURBO_NUM_WORKERS} --efficiency_mode ${efficiency_mode} --inference_type guide --test_all_pairs
