...
process inference_perturbo {
    ...
    script:
        ...
        echo "Using precomputed local-analysis PerTurbo results and MuData"
        cp --reflink=auto -f '${precomputed_element}' perturbo_local_analysis_per_element_output.tsv.gz
        cp --reflink=auto -f '${precomputed_guide}' perturbo_local_analysis_per_guide_output.tsv.gz
        cp --reflink=auto -f '${precomputed_mudata}' inference_mudata.h5mu
        """ : """
        # Run PerTurbo inference for per-element results
        perturbo_inference.py ${mudata} perturbo_local_analysis_per_element_output.tsv.gz --batch_size ${params.INFERENCE_PERTURBO_BATCH_SIZE} --num_workers 0 --efficiency_mode scaled --inference_type element

        # Run PerTurbo inference for per-guide results
        perturbo_inference.py ${mudata} perturbo_local_analysis_per_guide_output.tsv.gz --batch_size ${params.INFERENCE_PERTURBO_BATCH_SIZE} --num_workers 0 --efficiency_mode scaled --inference_type guide

        # Add both results to the base mudata file
        add_perturbo_results_to_mudata.py \\
            --per_guide_results perturbo_local_analysis_per_guide_output.tsv.gz \\
            --per_element_results perturbo_local_analysis_per_element_output.tsv.gz \\
            --base_mudata ${mudata} \\
